module blowfish{
    const MAXKEYBYTES:number = 56;		// 448 bits max
    const PLEN:number = 18;		// SBox passes
    const bf_P:Array<number> = [
    ...
    const SW:number = 4;
    const SLEN:number = 256;
    const bf_S:Array<Array<number>> = [
	[
    ...
    export class CBlowFish{
        private m_index:number = 0;
        private m_PArray:Array<number> = new Array<number>(PLEN);
        private m_SBoxes:Array<Array<number>> = new Array<Array<number>>();
        constructor(){
            for(let i:number = 0;i < PLEN ; ++i){
                this.m_PArray[i] = bf_P[i];
            }
            for(let i:number = 0;i < SW;++i){
                this.m_SBoxes.push(new Array<number>(SLEN));
                for(let j:number = 0;j < SLEN;++j){
                    this.m_SBoxes[i][j] = bf_S[i][j];
                }
            }
			this._print_array("constructor");
        }
		public _print_array(msg:string = "",b_force:boolean = false):void{
			if(b_force == false){
				return;
			}
			let output:string = "_print_array "+msg+" ";
			for(let i:number = 0;i < PLEN ; ++i){
				let c:number = this.m_PArray[i];
			...
			output = "";
			for(let i:number = 0;i < SW;++i){
				let ta:Array<number> = this.m_SBoxes[i];
				let cidx:number = 0;
                for(let j:number = 0;j < SLEN;++j){
                    let c:number = this.m_SBoxes[i][j];
					if(c < 0){
						c = c + 0xffffffff + 1;
					}
					output = output + " " + c.toString(16);
        ...
        private _bf_F(v:number):number{
            ...
            //
            //big endian
            let byte0:number = (v>>24)&0xFF;
            let byte1:number = (v>>16)&0xFF;
            let byte2:number = (v>>8)&0xFF;
            let byte3:number = (v)&0xFF;
            //
			//core.net_errlog("_bf_F ",v,byte0,byte1,byte2,byte3);
			let v1:number = this.m_SBoxes[0][byte0];
			let v2:number = this.m_SBoxes[1][byte1];
			let v3:number = this.m_SBoxes[2][byte2];
			let v4:number  =this.m_SBoxes[3][byte3];
			let tmp:number = ((v1+v2)^v3)+v4;
			//core.net_errlog("_bf_F1 ",tmp,v1,v2,v3,v4);
            let ret:number = ((this.m_SBoxes[0][byte0] + this.m_SBoxes[1][byte1])^this.m_SBoxes[2][byte2])+this.m_SBoxes[3][byte3];
            return ret;
        }
        private ROUND(a:number,b:number,idx:number):number{
            a ^= (this._bf_F(b) ^ this.m_PArray[idx]);
            return a;
        }
        private Blowfish_encipher(xl:number,xr:number):number[]{
            let TXL:number = xl;
            let TXR:number = xr;
        ...
        private Blowfish_decipher(xl:number,xr:number):number[]{
            let TXL:number = xl;
            let TXR:number = xr;
            let TXR_BACKUP = xr;
		...
		public testfunc():void{
			let testv:number = 0x2a8a33d7;
			let TXL:number = 0;
            let TXR:number = 0;

            TXL ^= testv;
            TXR = this.ROUND(TXR,TXL,1);
			let byte0:number = TXR&0xff;
			let byte1:number = (TXR>>8)&0xff;
			let byte2:number = (TXR>>16)&0xff;
			let byte3:number = (TXR>>24)&0xff;
			core.net_errlog("byte01234 ",byte0,byte1,byte2,byte3);
		}
        public InitializeKey(buff:Laya.Byte):void{
            let blen:number = buff.length;
            if(blen <= 0){
                return;
            }
            let j:number = 0;
            for(let i:number = 0;i < 18;++i){
                buff.pos = j;
                let byte0:number = buff.getUint8();
                buff.pos = (j+1)%blen;
                let byte1:number = buff.getUint8();
                buff.pos = (j+2)%blen;
                let byte2:number = buff.getUint8();
                buff.pos = (j+3)%blen;
                let byte3:number = buff.getUint8();
                //let temp:number = byte0|(byte1<<8)|(byte2<<16)|(byte3<<24);
				let temp:number = byte3|(byte2<<8)|(byte1<<16)|(byte0<<24);
            ...
            let datal:number = 0;
            let datar:number = 0;
            for(let i:number = 0;i < 18;i+=2){
                let bret:number[] = this.Blowfish_encipher(datal,datar);
                datal = bret[0];
                datar = bret[1];
            ...
            for(let i:number = 0;i < 4;++i){
                for(let j:number = 0;j < 256;j+=2){
                    let bret:number[] = this.Blowfish_encipher(datal,datar);
                    datal = bret[0];
                    datar = bret[1];
                    this.m_SBoxes[i][j] = datal;
                    this.m_SBoxes[i][j+1] = datar;
                }
            }
        ...
        public Initialize(seed:number):void
        {
            ...
            for( let i:number=0; i < 9; i++ )
            {
                ...
                if( seed & v )
                {
                    let temp:number = this.m_PArray[i*2];
            ...
            for( let i:number=9; i < 18; i++ )
            {
                ...
                if( seed & v )
                {
                    let temp:number = this.m_PArray[i-9];
                    this.m_PArray[i-9] = this.m_PArray[17-(i-9)];
                    this.m_PArray[17-(i-9)] = temp;
                }
            }
			this._print_array("Initialize seed1 "+this.m_index);
            for( let i:number=18; i < 32; i++ )
            {
                let v:number = 1 << i;
                if( seed & v )
                {
                    let t:number = i - 18;
                    let offset = (seed + this.m_index)&0xFF;
                    let offset2:number = ((seed >> 8) + t)&0xFF;
                    let temp:number = this.m_SBoxes[t & 3][offset];
        ...
        public Encode(buff:Laya.Byte):boolean{
			...
			//todo
            for(let i:number = 0;i < buff.length;i += 8){
				let opos:number = buff.pos;
				buff.pos = i;
                let xl:number = buff.getUint32();
                let xr:number = buff.getUint32();
                let bret:number[] = this.Blowfish_encipher(xl,xr);
				buff.pos = opos;
        ...
        public Decode(buff:Laya.Byte):boolean{
            ...
            for(let i:number = 0;i < buff.length;i += 8){
                ...
                let xl:number = buff.getUint32();
                let xr:number = buff.getUint32();
                let bret:number[] = this.Blowfish_decipher(xl,xr);
				buff.pos = opos;
                buff.writeUint32(bret[0]);
                buff.writeUint32(bret[1]);
            }
