module helper {
    ...
    export function create_format_link(text: string, htype: base.HYPERLINK_TYPE, u1: number, u2: number, u3: number, color: string = "#C18&"): string {
        if (text.length > 255) {
            text = text.substr(0, 255);
        }
        let len = text.length.toString(16);
        if (text.length < 16) {
            return color + '#H&l0' + len + '(' + text + ',' + htype + ',' + u1 + ',' + u2 + ',' + u3 + ')#D&';
        }
        return color + '#H&l' + len + '(' + text + ',' + htype + ',' + u1 + ',' + u2 + ',' + u3 + ')#D&';
    }
    export function trans_info(info: string): string {
        let temp: string = info;
        let markAb: string = "";
        let tempArr: string[] = temp.split('#');
        let markAbArr: string[] = [];
        ...
        let ele: string = "";
        for (let i = 0; i < markAbArr.length; i++) {
            ele = markAbArr[i];
    ...
    export function analysisMsgStr(msgString: string, dColor: string = "#FACD89", dFSize: string = "44") {
        var data = msgString;
        var htmlStr = "";
        // var dColor = "#FACD89";
        // var dFSize = "44";
    ...
    export function createSpanTag(color: string, fSize: string): string {
        let tag = '<span style="color:' + color + '; font-size:' + fSize + 'px;">';
        return tag;
    }
    export function createLinkTag(color: string, fSize: string, urlData: string, info: string): string {
        let tag = '<span href="' + urlData + '" style="color:' + color + '; font-size:' + fSize + 'px;">' + info + '</span>' + helper.createSpanTag(color, fSize);
        return tag;
    }
    export function createImgTag(imgUrl: string): string {
        let tag = '<img src="' + imgUrl + '"/>';
        return tag;
    }
